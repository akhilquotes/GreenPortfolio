...
import { searchStock, getGlobalQuote } from "../Lib/StocksApi";
...
const Portfolio = ({
  ...
  const [snpCo2e, setSnpCo2e] = useState(0.3);
  ...
  const getStockPerformanceChartData = () => {
    ...
    for (let stock of portfolio) {
      if (result.filter((x) => x.Date === stock.TransactionDate).length === 0) {
        ...
        let reqPrice = 0;
        let co2e = 0;
        let totalCostofReqStocks = getTotalCost(groupPortfolio(reqStocks));
        for (let s of reqStocks) {
          let multiplier = s.Currency === "INR" ? 1 / 80 : 1;
          let multiplierCo2e = s.Currency === "INR" ? 80 : 1;
          let costOfStocks = parseInt(s.Quantity) * parseFloat(s.Price);
          let weightage = costOfStocks / totalCostofReqStocks;
          reqPrice += costOfStocks * multiplier;
          co2e += parseFloat(
            (weightage * parseFloat(s.CO2E) * multiplierCo2e).toFixed(5)
          );
        }
        let obj = {
      ...
      for (let coin of greenCoins) {
        if (
          gcResult.filter((x) => x.Date === coin.TransactionDate).length === 0
        ) {
          let reqCoins = greenCoins.filter(
            (x) => new Date(x.TransactionDate) <= new Date(coin.TransactionDate)
          );
          let co2e = 0;
          for (let s of reqCoins) {
            co2e += parseFloat((-1 * s.Coins * parseFloat(s.Co2e)).toFixed(2));
          }
          let obj = {
            Date: coin.TransactionDate,
            Co2e: co2e,
          };
          gcResult.push(obj);
        }
  ...
  const getTotalGains = () => {
    ...
    for (let data of grpData) {
      ...
      let multiplier = data.Currency === "INR" ? 1 / 80 : 1;
      let multiplierCo2e = data.Currency === "INR" ? 80 : 1;
      totalChange += data.TotalCost * perStockChange * multiplier;
      totalCO2e += data.CO2E * multiplierCo2e * weightage;
    }
    setCard1val((totalPercentChange * 100).toFixed(2));
    setCard2val(totalChange.toFixed(2) + " USD");
    setCard3val(totalCO2e.toFixed(5) + " MT/USD");
  };
  ...
  return (
    <>
        ...
        <Col xs={8}>
          <Row>
            ...
            <Col xs={3}>
              <StatusCard
                bgColor="bg-warning"
                title="CO2e Portfolio"
                text={card3Val}
                textColor={
                  parseFloat(card3Val) < snpCo2e
                    ? "text-success"
                    : "text-danger"
                }
              />
      ...
      <Row>
        ...
        <Col xs={4} className="ms-0">
          <StockPerformanceChart
            perfData={stockPerfData}
            keyCol="Co2e"
            title="Carbon Emissions of Portfolio"
        ...
        <Col xs={4} className="ms-0">
          <StockPerformanceChart
            perfData={gcData}
            keyCol="Co2e"
            title="Carbon Offsetting by Green Coin Donations"
            series="Carbon footprint offset in MT"
            color="#f2a852"
          />
        </Col>
      </Row>
